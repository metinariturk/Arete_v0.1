...
class Rent extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->viewFolder = "rent_v";
        $this->load->model("Rent_model");
        $this->load->model("Rent_file_model");

        $this->load->model("Vehicle_model");
        $this->load->model("Settings_model");
        $this->load->model("Company_model");

        $this->Module_Name = "Rent";
        $this->Module_Title = "Araç Kiralama";

        // Folder Structure
        $this->Upload_Folder = "uploads";
        $this->Module_Main_Dir = "vehicle_v";
        $this->Module_Depended_Dir = "vehicle";
        $this->Module_File_Dir = "Rent";
        $this->Module_Table = "Rent";
    ...
    public function index()
    {
        ...
        $items = $this->Rent_model->get_all(array());
    ...
    public function file_form($id)
    {
        ...
        $viewData->item = $this->Rent_model->get(
            array(
                "id" => $id
            )
        );

        $viewData->item_files = $this->Rent_file_model->get_all(
            array(
    ...
    public function save($vehicle_id = null)
    {
        ...
        $this->form_validation->set_rules("kiralama_turu", "Kiralama Türü", "required|trim");
        $this->form_validation->set_rules("baslangic", "Kiralama Başlangıç Tarih", "required|trim"); //2
        $this->form_validation->set_rules("bitis", "Kiralama Bitiş Tarih", "callback_baslangic_bitis[$baslangic]|required|trim"); //2
        $this->form_validation->set_rules("sure", "Kiralama Süresi", "required|trim"); //2
        $this->form_validation->set_rules("fiyat", "Fiyat", "required|trim"); //2
        $this->form_validation->set_rules("kiralayan_firma", "Kiralayan Firma", "required|trim"); //2
        $this->form_validation->set_rules("kiralanan_firma", "Kiralanan Firma", "required|trim"); //2
        $this->form_validation->set_rules("yakit", "Yakıt", "required|trim"); //2
        $this->form_validation->set_rules("operator", "Operatör", "required|trim"); //4
        $this->form_validation->set_rules("bakim_servis", "Servis Bakım Arıza", "required|trim"); //4
        ...
        if ($validate) {
            ...
            $path = "$this->File_Dir_Prefix/$vehicle_id/kiralama";
            ...
            $insert = $this->Rent_model->add(
                array(
                    "vehicle_id"        => $vehicle_id,
                    "kiralama_turu"     => $this->input->post("kiralama_turu"),
                    "baslangic"         => $baslangic,
                    "bitis"             => $bitis,
                    "sure"              => $this->input->post("sure"),
                    "fiyat"             => $this->input->post("fiyat"),
                    "kiralayan_firma"   => $this->input->post("kiralayan_firma"),
                    "kiralanan_firma"   => $this->input->post("kiralanan_firma"),
                    "yakit"             => $this->input->post("yakit"),
                    "operator"          => $this->input->post("operator"),
                    "bakim_servis"      => $this->input->post("bakim_servis"),
                    "isActive"          => "1",
            ...
            if ($insert) {
                ...
                $alert = array(
                    ...
                    "text" => "Kiralama bilgileri başarılı bir şekilde eklendi",
                    "type" => "success"
                );
                ...
                $alert = array(
                    ...
                    "text" => "Kiralama bilgileri ekleme sırasında bir problem oluştu",
                    "type" => "danger"
                );
            }
            ...
            $alert = array(
                "title" => "İşlem Başarısız",
                "text" => "Kiralama bilgileri ekleme formunda hata var",
                "type" => "danger"
            );
    ...
    public function delete($id)
    {
        ...
        $delete1 = $this->Rent_file_model->delete(
            array(
                "$this->Dependet_id_key"    => $id
            )
        );
        $delete2 = $this->Rent_model->delete(
            array(
                "id" => $id
            )
        );
    ...
    public function file_upload($id, $vehicle_id)
    {
        ...
        $config["upload_path"] = "$this->File_Dir_Prefix/$vehicle_id/kiralama/";
...
"createdBy" => active_user_id(),
    ...
    {
        $fileName = $this->Rent_file_model->get(
            array(
                "id"    => $id
            )
        );
        ...
        $rent_id = get_from_id("rent_files","rent_id", $id);
        $vehicle_id = get_from_id("rent","vehicle_id", $rent_id);


        $file_path = "$this->File_Dir_Prefix/$vehicle_id/kiralama/$fileName->img_url";
    ...
    public function fileDelete($id, $from){
        ...
        $delete = $this->Rent_file_model->delete(
            array(
                "id"    => $id
            )
        );
        ...
        if($delete){
            ...
            $path = "$this->File_Dir_Prefix/$vehicle_id/kiralama/$fileName->img_url";

            unlink($path);

            redirect(base_url("$this->Module_Name/$from/$rent_id"));
    ...
    public function fileDelete_all($id, $from){
        ...
        echo $vehicle_id = get_from_id("rent", "vehicle_id",$id);
        ...
        if($delete){
            ...
            $dir_files = directory_map("$this->File_Dir_Prefix/$vehicle_id/kiralama");

            foreach($dir_files as $dir_file){
                unlink("$this->File_Dir_Prefix/$vehicle_id/kiralama/$dir_file");
            }
