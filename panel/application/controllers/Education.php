...
class Education extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->viewFolder = "education_v";
        $this->load->model("Education_model");
        $this->load->model("Education_file_model");
    ...
    public function index()
    {
        ...
        $items = $this->Education_model->get_all(array());
    ...
    public function new_form($wid = null, $edu_type = null)
    {
...
$users = $this->User_model->get_all(array(
    ...
    public function save($id)
    {
        ...
        $file_name = "EDU-".$this->input->post('dosya_no');

        $issue_date = dateFormat('Y-m-d', $this->input->post("egitim_tarihi"));

        $this->load->library("form_validation");

        $this->form_validation->set_rules("dosya_no", "Dosya No", "greater_than[0]|is_unique[education.dosya_no]|required|trim|exact_length[$file_name_len]|callback_duplicate_code_check");
        $this->form_validation->set_rules("belge_no", "Belge Numarası", "required|trim");
        $this->form_validation->set_rules("egitim_turu", "Eğitim Türü", "required|trim");
        $this->form_validation->set_rules("egitim_tarihi", "Eğitimin Verildiği Tarih", "required|trim");
        $this->form_validation->set_rules("gecerlilik_tarihi", "Geçerlilik Tarihi", "callback_issue_valid_date[$issue_date]|required|trim");
        $this->form_validation->set_rules("egitim_veren", "Eğitim Veren", "required|trim");
        ...
        if ($validate) {
            ...
            if ($contract_id == 0) {
                ...
                $path = "$this->Upload_Folder/project_v/$project_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$file_name";
            } else {
                $project_id = project_id_cont($contract_id);
                $contract_code = get_from_id("contract", "dosya_no", $contract_id);
                $project_code = project_code($project_id);
                $path = "$this->Upload_Folder/project_v/$project_code/$contract_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$file_name";
            }
            ...
            $insert = $this->Education_model->add(
                array(
                    ...
                    "egitim_turu"       => $this->input->post("egitim_turu"),
                    "egitim_tarihi"     => $egitim_tarihi,
                    "gecerlilik_tarihi" => $gecerlilik_tarihi,
                    "egitim_veren"      => $this->input->post("egitim_veren"),
...
$users = $this->User_model->get_all(array(
            ...
            $viewData->item = $this->Education_model->get(
                array(
                    "id" => $id
                )
            );

            $viewData->item_files = $this->Education_file_model->get_all(
                array(
...
$users = $this->User_model->get_all(array(
    ...
    {
        $education_code = get_from_id("education","dosya_no","$id");
        $site_id = get_from_id("education","site_id","$id");
        $contract_id = get_from_id("site","contract_id","$site_id");
        $site_code = get_from_id("site","dosya_no","$site_id");
        $safety_id = get_from_id("education","safety_id","$id");
        $safety_code = get_from_id("safety","dosya_no","$safety_id");
        $personel_folder = convertToSEO(worker_name($id));
        ...
        if ($contract_id == 0) {
            $project_id = get_from_id("site", "proje_id", $site_id);
            $project_code = project_code($project_id);
            $path = "$this->Upload_Folder/project_v/$project_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$education_code";
        } else {
            $project_id = project_id_cont($contract_id);
            $contract_code = get_from_id("contract", "dosya_no", $contract_id);
            $project_code = project_code($project_id);
            $path = "$this->Upload_Folder/project_v/$project_code/$contract_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$education_code";
        }
        ...
        $delete1 = $this->Education_file_model->delete(
            array(
                "$this->Dependet_id_key" => $id
            )
        );

        $delete2 = $this->Education_model->delete(
            array(
                "id" => $id
            )
        );
...
"deletedBy" => active_user_id(),
    ...
    public function file_upload($id)
    {
        ...
        $safety_id = get_from_id("education","safety_id","$id");
        $safety_code = get_from_id("safety","dosya_no","$safety_id");
        $worman_id = get_from_id("education","worker_id","$id");
...
"createdBy" => active_user_id(),
    ...
    {
        $fileName = $this->Education_file_model->get(
            array(
                "id"    => $id
            )
        );

        $education_id= $fileName->education_id;

        $education_code = get_from_id("education","dosya_no","$education_id");
        $site_id = get_from_id("education","site_id","$education_id");
        $contract_id = get_from_id("site","contract_id","$site_id");
        $site_code = get_from_id("site","dosya_no","$site_id");
        $safety_id = get_from_id("education","safety_id","$education_id");
        $safety_code = get_from_id("safety","dosya_no","$safety_id");
        $worman_id = get_from_id("education","worker_id","$education_id");
        $personel_folder = convertToSEO(worker_name($worman_id));
    ...
    public function fileDelete($id, $from){
        ...
        if ($contract_id == 0) {
            $project_id = get_from_id("site", "proje_id", $site_id);
            $project_code = project_code($project_id);
            $path = "$this->Upload_Folder/project_v/$project_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$education_code/$fileName->img_url";
        } else {
            $project_id = project_id_cont($contract_id);
            $contract_code = get_from_id("contract", "dosya_no", $contract_id);
            $project_code = project_code($project_id);
            $path = "$this->Upload_Folder/project_v/$project_code/$contract_code/site/$site_code/safety/$safety_code/personel/$personel_folder/educations/$education_code/$fileName->img_url";
        }


        $delete = $this->Education_file_model->delete(
            array(
                "id"    => $id
            )
        );
        ...
        if($delete){
            ...
            redirect(base_url("$this->Module_Name/$from/$education_id"));
