...
class Cost extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->viewFolder = "cost_v";
        $this->load->model("Cost_model");
        $this->load->model("Cost_file_model");
    ...
    public function index()
    {
        ...
        $items = $this->Cost_model->get_all(array());
    ...
    public function update_form($id)
    {
        ...
        $viewData->item = $this->Cost_model->get(
            array(
                "id" => $id
            )
        );

        $viewData->item_files = $this->Cost_file_model->get_all(
            array(
    ...
    public function save($id)
    {
        ...
        $file_name = "YM-" . $this->input->post('dosya_no');

        $this->load->library("form_validation");

        $this->form_validation->set_rules("dosya_no", "Dosya No", "greater_than[0]|is_unique[cost.dosya_no]|required|trim|exact_length[$file_name_len]|callback_duplicate_code_check");
        $this->form_validation->set_rules("ym_grup", "Yaklaşık Maliyet Grubu", "required|trim");
        $this->form_validation->set_rules("ym_ad", "Yaklaşık Maliyet Adı", "required|trim");
        $this->form_validation->set_rules("cost", "Yaklaşık Maliyet Tutarı", "required|trim");
        ...
        if ($validate) {
            ...
            $insert = $this->Cost_model->add(
                array(
                    "dosya_no" => $file_name,
                    "auction_id" => $id,
                    "ym_grup" => $this->input->post("ym_grup"),
                    "ym_ad" => $this->input->post("ym_ad"),
                    "poz_no" => $this->input->post("poz_no"),
...
"createdBy" => active_user_id(),
    ...
    public function update($id)
    {
        ...
        $this->form_validation->set_rules("ym_grup", "Proje Çizim Grubu", "required|trim");
        $this->form_validation->set_rules("ym_ad", "Çizim Ad", "required|trim");
        $this->form_validation->set_rules("cost", "Yaklaşık Maliyet Tutarı", "required|trim");
    ...
    public function delete($id)
    {
        ...
        $auction_id = get_from_id("cost", "auction_id", $id);
        $project_id = get_from_id("auction", "proje_id", $auction_id);
        $project_code = project_code($project_id);
        $auction_code = get_from_id("auction", "dosya_no", $auction_id);
        $Cost_code = get_from_id("cost", "dosya_no", $id);

        $path = "$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code/";
        ...
        $delete1 = $this->Cost_file_model->delete(
            array(
                "$this->Dependet_id_key" => $id
            )
        );

        $delete2 = $this->Cost_model->delete(
            array(
                "id" => $id
            )
        );
...
"deletedBy" => active_user_id(),
    ...
    public function file_upload($id)
    {
        ...
        $project_id = get_from_id("auction", "proje_id", $auction_id);
        $project_code = project_code($project_id);
        $auction_code = get_from_id("auction", "dosya_no", get_from_id("cost", "auction_id", $id));
        $Cost_code = get_from_id("cost", "dosya_no", $id);

        $config["allowed_types"] = "*";
        $config["upload_path"] = "$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code";
...
"createdBy" => active_user_id(),
    ...
    {
        $fileName = $this->Cost_file_model->get(
            array(
                "id" => $id
            )
        );

        $cost_id = get_from_id("cost_files", "cost_id", $id);
        $auction_id = get_from_id("cost", "auction_id", $cost_id);
        $project_id = get_from_id("auction", "proje_id", $auction_id);
        $project_code = project_code($project_id);
        $auction_code = get_from_id("auction", "dosya_no", $auction_id);
        $Cost_code = get_from_id("cost", "dosya_no", $cost_id);

        $file_path = "$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code/$fileName->img_url";
    ...
    public function fileDelete($id)
    {
        ...
        $delete = $this->Cost_file_model->delete(
            array(
                "id" => $id
            )
        );
        ...
        if ($delete) {
            ...
            $path = "$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code/$fileName->img_url";
    ...
    public function fileDelete_all($id)
    {
        ...
        if ($delete) {
            ...
            $dir_files = directory_map("$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code");

            foreach ($dir_files as $dir_file) {
                unlink("$this->File_Dir_Prefix/$project_code/$auction_code/$this->File_Dir_Suffix/$Cost_code/$dir_file");
            }
    ...
    public function download_all($Cost_id)
    {
        ...
        $path = "uploads/project_v/$project_code/$auction_code/$this->Module_Name/$Cost_code";
        echo $path;
