        ...
        <table class="table">
            ...
            <?php if (!empty($sartnameler)) { ?>
                <?php foreach ($sartnameler as $sartname) { ?>
                    <tr data-toggle="collapse" data-target="#accordion<?php echo $sartname->id; ?>" class="clickable"
                        id="center_row">
                        <td>
                            <a data-tooltip-location="right"
                               href="">
                                <?php echo $sartname->id; ?>
                            </a>
                        </td>
                        <td><?php echo condition_type($sartname->condition_type); ?></td>
                        <td><?php echo $sartname->aciklama; ?></td>
                        <td>
                            <a class="pager-btn btn btn-info btn-outline" onclick="page_forward(this)" data-text="Bu Şartname"
                               data-note="Sayfadan Çıkmak Üzeresiniz"
                               data-url="<?php echo base_url("condition/file_form/$sartname->id"); ?>">
            ...
            <?php if (empty($idari_sart)) { ?>
                <tr>
                    <td colspan="4">
                        <a class="pager-btn btn btn-purple btn-outline"
                           href="<?php echo base_url("condition/new_form/auction_display/$item->id/1"); ?>">
            ...
            <?php } ?>
            <?php if (empty($teknikler)) { ?>
                <tr>
                    <td colspan="4">
                        <a class="pager-btn btn btn-purple btn-outline"
                           href="<?php echo base_url("condition/new_form/auction_display/$item->id/2"); ?>">
                            <i class="fa fa-plus"></i>
            ...
            <?php if (empty($isgler)) { ?>
                <tr>
                    <td colspan="4">
                        <a class="pager-btn btn btn-purple btn-outline"
                           href="<?php echo base_url("condition/new_form/auction_display/$item->id/3"); ?>">
                            <i class="fa fa-plus"></i>
                            İSG Şartname Ekle
                        </a>
                    </td>
...
<div class="col-sm-4">
    ...
    <table class="table">
        ...
        <tr>
            <th colspan="3">
                <h3>Şartname Kriterleri</h3>
            </th>
        </tr>
        <tr>
            <?php if (empty($idari_sart)) { ?>
                <th>
                    İdari Şartname Ekleyiniz
                </th>
            <?php } ?>
        </tr>
        </thead>
        <?php if (!empty($idari_sart)) { ?>
            <?php foreach ($idari_sart
