    ...
    <div class="card-body">
        <div class="mb-2">
            ...
            <input class="datepicker-here form-control digits <?php cms_isset(form_error("tahsilat_tarih"), "is-invalid", ""); ?>"
                   type="text"
                   name="tahsilat_tarih"
                   value="<?php echo isset($form_error) ? set_value("tahsilat_tarih") : dateFormat('d-m-Y',$item->tahsilat_tarih); ?>"
        ...
        <div class="mb-2">
            <div class="col-form-label">Tahsilat Tutar
                <?php if (isset($form_error)) { ?>
                    <?php if (!empty(form_error("tahsilat_miktar"))) { ?>
                        <div style="color: red">*** Sözleşme bedelinden fazla tahsilat ödemesi yapılamaz. Özel bir gerekçe ile fazla tahsilat verilmesi gerekiyorsa aşağıdaki onay kutusunu işaretleyiniz.
                            <br><input  name="onay" type="checkbox" id="cb-10"> Sözleşme bedelinden fazla tahsilat ödemesi yapmak istiyorum!</div>
                    <?php } ?>
                <?php } ?>
            </div>

            <input type="integer" id="calA" onblur="calcular()" onfocus="calcular()"
                   onChange="myFunction(calA)" class="form-control" name="tahsilat_miktar"
                   placeholder="Tahsilat Tutar"
                   value="<?php echo isset($form_error) ? set_value("tahsilat_miktar") : "$item->tahsilat_miktar"; ?>">
            <?php if (isset($form_error)) { ?>
        ...
        <div class="mb-2">
            ...
            <select id="select2-demo-1" style="width: 100%;"
                ...
                <option selected="selected"
                        value="<?php echo isset($form_error) ? set_value("tahsilat_turu") : $item->tahsilat_turu; ?>"><?php echo isset($form_error) ? set_value("tahsilat_turu") : $item->tahsilat_turu; ?>
                </option>
        ...
        <div class="mb-2">
            ...
            <input class="datepicker-here form-control digits <?php cms_isset(form_error("vade_tarih"), "is-invalid", ""); ?>"
                   type="text"
                   name="vade_tarih"
                   value="<?php echo isset($form_error) ? set_value("vade_tarih") : dateFormat_dmy($item->vade_tarih); ?>"
                   data-options="{ format: 'DD-MM-YYYY' }"
                   data-language="tr">
            <?php if (isset($form_error)) { ?>
