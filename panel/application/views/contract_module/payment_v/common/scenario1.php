<script>
    function calcular() {
        ...
        var valorI = Number(document.getElementById('I').value, 10); // Avans Mahsup
        var valorI_s = valorA / valorI; // Avans Mahsup Oranı Hesapla
        document.getElementById('I_s').value = valorI_s.toFixed(2); // Avans Mahsup Yaz

        document.getElementById('I').addEventListener('input', function() {
            valorI = Number(document.getElementById('I').value, 10);
            valorI_s = valorA / valorI; // Avans Mahsup Oranı Hesapla
            document.getElementById('I_s').value = valorI_s.toFixed(2); // Avans Mahsup Yaz
        });

        document.getElementById('I_s').addEventListener('input', function() {
            var valorI_s = Number(document.getElementById('I_s').value, 10); // Avans Mahsup Oranı Seç
            var valorI = valorI_s * valorA / 100; // Avans Mahsup Hesapla
        ...
        valorKES_a = valorKES_a_s * (valorE - valorI) / 100; // Stopaj Oranı Hesap
        <?php } else { ?>
        valorKES_a = valorKES_a_s * valorE / 100; // Stopaj Oranı Hesap
        <?php } ?>
