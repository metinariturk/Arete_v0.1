...
function contract_auth($id)
{
    $ci = &get_instance();
    $ci->load->database();

    $ci->db->select('isveren_sorumlu,isveren_yetkili,yuklenici_yetkili,yuklenici_sorumlu');
    $ci->db->from('contract');
    $ci->db->where('id', $id);
    $query = $ci->db->get();
    ...
    if ($query->num_rows() > 0) {
        $row = $query->row();
        $auth_user = $row->isveren_sorumlu . ',' . $row->isveren_yetkili . ',' . $row->yuklenici_yetkili . ',' . $row->yuklenici_sorumlu;
        return str_getcsv($auth_user);
    } else {
        $auth_user = array();
        return str_getcsv($auth_user);
    }
...
function contract_company($id)
{
    $ci = &get_instance();
    $ci->load->database();

    $ci->db->select('isveren,isveren_yetkili,yuklenici');
    $ci->db->from('contract');
    $ci->db->where('id', $id);
    $query = $ci->db->get();
    ...
    if ($query->num_rows() > 0) {
        $row = $query->row();
        $auth_user = $row->isveren . ',' . $row->yuklenici;
        return str_getcsv($auth_user);
    } else {
        $auth_user = array();
        return str_getcsv($auth_user);
...
function site_auth($id)
{
    ...
    $ci->db->select('santiye_sefi,teknik_personel');
    $ci->db->from('site');
    $ci->db->where('id', $id);
    $query = $ci->db->get();
    ...
    if ($query->num_rows() > 0) {
        $row = $query->row();
        $auth_user = $row->santiye_sefi . ',' . $row->teknik_personel;
        return str_getcsv($auth_user);
    } else {
        $auth_user = array();
        return str_getcsv($auth_user);
...
function auction_auth($id)
{
    $ci = &get_instance();
    $ci->load->database();

    $ci->db->select('yetkili_personeller');
    $ci->db->from('auction');
    $ci->db->where('id', $id);
    $query = $ci->db->get();
    ...
    if ($query->num_rows() > 0) {
        $row = $query->row();
        $auth_user = $row->yetkili_personeller;
        return str_getcsv($auth_user);
    } else {
        $auth_user = array();
        return str_getcsv($auth_user);
